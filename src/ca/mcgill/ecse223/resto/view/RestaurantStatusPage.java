...
public class RestaurantStatusPage extends JFrame {
	...
	private String stringReservations(List<Reservation> reservations) {
		...
		for (Reservation res : reservations) {
			...
			returned += ("Name:" + res.getContactName() + " - Phone" + res.getContactPhoneNumber() + "<br>");
		}
		return returned;
	}
	...
	private String stringTables(List<Table> tables) {
		...
		for (Table t : tables) {
			returned += ("" + tables.indexOf(t) + " : " + "<br>");
			returned += ("---N" + t.getNumber() + " - L" + t.getLength() + " - W" + t.getWidth() + "---<br>");
			returned += ("---X" + t.getX() + " - Y" + t.getY() + " - S" + t.getSeats().size() + "---<br>");
		}
		return returned;
	}
	...
	private int getSeatPosition(Seat s) {
		return s.getTable().getSeats().indexOf(s);
	}
